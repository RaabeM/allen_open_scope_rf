...
def load_rf_maps(
    ...
    ----------
    best_params :
        Path to best_params.csv produced by optimize_parameters, or the
        DataFrame itself (as returned by optimize_parameters).
    ...
    """
    if not isinstance(best_params, pd.DataFrame):
...
def main():
    ...
    df = optimize_parameters(args.results_dir)
    df = load_rf_maps(df)
